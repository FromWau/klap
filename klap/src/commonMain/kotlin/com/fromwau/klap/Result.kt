...
fun <S> Ok(value: S): Result<S, Nothing> = Result.Success(value)

/** Convenience builder: wrap an error as a failed result. */
fun <E> Err(error: E): Result<Nothing, E> = Result.Error(error)

inline fun <S, E, T> Result<S, E>.map(transform: (S) -> T): Result<T, E> = when (this) {
...
inline fun <S, E> Result<S, E>.getOrElse(fallback: (E) -> S): S = when (this) {
    is Result.Success -> value
    is Result.Error -> fallback(error)
}

inline fun <S, E, T> Result<S, E>.fold(onSuccess: (S) -> T, onError: (E) -> T): T = when (this) {
